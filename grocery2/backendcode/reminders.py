...
sender_email = "nishkarshs33@gmail.com"
...
reminder_time = datetime.now().replace(
    hour=19, minute=19, second=0, microsecond=0)
...
def check_user_activity(user):
    ...
    user_orders_today = Order.query.filter_by(
        user_id=user.id).filter(Order.buy_at >= today).all()
